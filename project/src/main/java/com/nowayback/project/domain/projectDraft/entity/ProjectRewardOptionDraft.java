...
@Entity
@Table(name = "p_project_reward_option_draft")
@Getter
@NoArgsConstructor(access = AccessLevel.PROTECTED)
public class ProjectRewardOptionDraft {
    ...
    private void validateAdditionalPrice(Integer additionalPrice) {
        if (additionalPrice != null && additionalPrice < 0) {
            throw new ProjectDomainException(ProjectDomainErrorCode.INVALID_REWARD_OPTION_PRICE);
        }
    }

    private void validateStock(Integer stockQuantity) {
        if (stockQuantity != null && stockQuantity < 0) {
            throw new ProjectDomainException(ProjectDomainErrorCode.INVALID_REWARD_OPTION_STOCK);
        }
    }
    ...
    private void validateDisplayOrder(Integer displayOrder) {
        if (displayOrder != null && displayOrder < 0) {
            throw new ProjectDomainException(
                ProjectDomainErrorCode.INVALID_REWARD_OPTION_DISPLAY_ORDER);
        }
    }
    ...
    public void validateForSubmission() {
        ...
        if (additionalPrice == null || additionalPrice < 0) {
            errors.add(ProjectDomainErrorCode.INVALID_REWARD_OPTION_PRICE.getMessage());
        }
        if (stockQuantity == null || stockQuantity < 0) {
            errors.add(ProjectDomainErrorCode.INVALID_REWARD_OPTION_STOCK.getMessage());
        }
        if (displayOrder == null || displayOrder < 0) {
            errors.add(ProjectDomainErrorCode.INVALID_REWARD_OPTION_DISPLAY_ORDER.getMessage());
        }

        if (!errors.isEmpty()) {
            throw new ProjectDomainException(
                ProjectDomainErrorCode.INVALID_REWARD_OPTION_DRAFT_SUBMISSION);
        }
    }
