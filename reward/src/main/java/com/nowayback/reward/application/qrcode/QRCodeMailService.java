...
public class QRCodeMailService {
     ...
     * QR 코드 발급 이메일 발송
     */
    public void sendQRCodeEmail(String email, UUID qrCodeId) {
        try {
            MimeMessage message = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(message, true, "UTF-8");

            helper.setTo(email);
            helper.setSubject("입장권 QR 코드가 발급되었습니다");
            helper.setText(buildQRCodeEmailContent(qrCodeId), true);
    ...
    private String buildQRCodeEmailContent(UUID qrCodeId) {
        return String.format("""
                <html>
                <body>
                    <h2>입장권 QR 코드가 발급되었습니다!</h2>
                    <p>펀딩 성공을 축하드립니다.</p>
                    <div>
