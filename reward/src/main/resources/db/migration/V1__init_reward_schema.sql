...
CREATE INDEX idx_processed_event_project_id ON p_idempotent_keys(project_id);
CREATE INDEX idx_processed_event_processed_at ON p_idempotent_keys(processed_at);
...
COMMENT ON TABLE p_idempotent_keys IS '프로젝트 이벤트 중복 처리 방지 테이블';